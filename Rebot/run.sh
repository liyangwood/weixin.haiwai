...
runGoogleEWCNoRunning(){
    ...
    ps -ef |grep meteor|grep 8001 |awk '{print $2}'|xargs sudo kill -9
    ps -ef |grep meteor|grep 8002 |awk '{print $2}'|xargs sudo kill -9
    ...
    sudo -E meteor run --settings settings.json --port $PORT &
